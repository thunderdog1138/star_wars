...
minetest.register_craftitem("rangedweapons:python_uld", {
	...
	on_secondary_use = function(itemstack, player, pointed_thing)
	   ...
	   if inv:contains_item("main", "rangedweapons:357") then
		  ...
		  minetest.after(0.3,
						 function(player, wield_index)
							player:get_inventory():set_stack("main", wield_index, "rangedweapons:python")
							minetest.sound_play("rangedweapons_reload_a", {player})
						 end,
						 player,wield_index
		  )
		  return ItemStack("rangedweapons:python_rld")
	   end
	end
})
